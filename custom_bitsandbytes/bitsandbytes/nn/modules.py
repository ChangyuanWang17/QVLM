...
class Linear4bit(nn.Linear):
    def __init__(self, input_features, output_features, bias=True, compute_dtype=None, compress_statistics=True, quant_type='fp4',device=None):
        ...
        # print("before"+str(self.weight.data.shape))
        self.weight = Params4bit(self.weight.data, requires_grad=False, compress_statistics=compress_statistics, quant_type=quant_type)
        # print("after"+str(self.weight.data.shape))
        ...
        # print("*********************************************")
        # print("using qlora fake activation "+str(self.activation_bit)+"bit, llama layer "+str(self.llama_layer)+", block"+str())
        print("using qlora fake activation {}bit, llama layer {}, block {},{}".format(
            self.activation_bit, self.llama_layer, self.count_block, self.count_layer))
        # print("*********************************************")
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print(x.shape, self.weight.shape)  # torch.Size([1, 1, 4096]) torch.Size([8388608, 1])
        # quant_x = x
        '''global count_block, count_layer
        if x.shape[1] != 1 and x.shape[1] != 257:
            count_layer += 1
            if count_layer == 8:
                count_layer = 1
                count_block += 1
            if count_block == 33:
                count_block = 1
            print(count_block, count_layer, x.shape)
        if x.shape[1] != 1 and x.shape[1] != 257:
            count_layer += 1
            if count_layer == 8:
                count_layer = 1
        ...
        if count_block >= 75:
            return 0'''
        # if count_block % 32 == 0 and count_layer == 1:
        #     print(x[0][:][0:16].max())
        #     print(x[0][:][0:16])
        ...
        '''if x.shape[1] == 1:
            ...
            quant_x = self.quant_activation(x)'''
        # quant_x = x
        quant_x = self.quant_activation(x)
        # print(quant_x, x)
        # print("**************")
        # print(torch.min(x, dim=-1)[0].squeeze(dim=0), torch.max(x, dim=-1)[0].squeeze(dim=0))
        ...
        # out = F.linear(x, weight=self.weight.t().reshape(x.shape[1],-1), bias=bias)
